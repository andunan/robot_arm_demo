        ...
        while True:
            try:
                ...
                for angle in self.arm.joint_angles:
                    send_data += float_to_bytes(angle * 180.0 / math.pi)
                # print(send_data)
                # send data
                ...
                if cmd_type == 0: # connect message
                    ...
                    try:
                        for i, joint_id in enumerate(self.arm.joint_ids):   
                            tmp_angle_deg = bytes_to_float(data[1 + i * 4: 1 + (i + 1) * 4], '<')
                            tmp_angle_rad = tmp_angle_deg * math.pi / 180.0
                            self.servo_angle_list[joint_id - 1] = tmp_angle_rad
